"""
    "Inspired" by https://github.com/phamquiluan/table-transformer/blob/main/core.py 
        which made simple inference easier
"""
import sys
import logging
import torch
...
def set_cell_text(cells, image, clean=False):
    for cell in cells:
        ...
        #   OCR
        text = image_to_string(roi, lang='deu') #   TODO: here you should use self.lang
        if clean:
            text.strip()
            text = text.replace('\n', '')
        cell['content'] = text
...
class TableInformer:
    def __init__(self, weight_path):
        #   NOTE: in the example they here assert if the weight path exists
        args = load_args("./src/structure_config.json")
    ...
    def predict(self, image_path, debug=True, threshold=0.7):
        """
        ...
        set_cell_text(cells, image, clean=True)
